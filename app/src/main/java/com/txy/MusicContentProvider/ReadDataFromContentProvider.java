package com.txy.MusicContentProvider;
...
public class ReadDataFromContentProvider {
    ...
    public static List<MyMusic> readAudio(Context context) {
        ContentResolver resolver = context.getContentResolver() ;
        ...
        if(null != cursor && cursor.getCount() > 0) {
            ...
            while(cursor.moveToNext()) {
                ...
                //////////////////
                data.add(new MyMusic(cursor.getString(0), cursor.getString(1), cursor.getString(2), cursor.getString(3) , cursor.getLong(4) , bmp)) ;
            }
        }
        if(null != cursor) {
            cursor.close() ;
